...
export default function DashboardPage() {
  ...
  const KPIData = [
    ...
    },
    {
      ...
      desc: 'Módulo Financeiro', icon: AlertCircle, color: 'text-gray-600', iconColor: 'text-gray-600'
    },
    {
      title: 'Inadimplência', value: '—',
  ...
  return (
    <div className="flex flex-col flex-1 w-full min-w-0 text-white">
      ...
      <div className="px-4 md:px-6 py-6 space-y-6 max-w-[1500px] mx-auto w-full pb-20 fade-slide-up">
        ...
        {/* ── Main chart ─────────────────────────────────── */}
        <div className="glass-card rounded-[24px] p-6 border border-white/10 relative overflow-hidden fade-slide-up" style={{ animationDelay: '280ms' }}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowComparison(p => !p)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${showComparison ? 'bg-[#DC143C]/20 border-[#DC143C]/30 text-[#DC143C]' : 'border-white/10 text-gray-500 hover:text-white'}`}>
                Pres. vs Faltas
              </button>
              ...
              <div className="flex items-center gap-1 bg-black/40 rounded-lg p-1 border border-white/5">
                {['Semana', 'Mês', 'Ano'].map(v => (
                  <button key={v} onClick={() => setPeriod(v)}
                    className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${period === v ? 'bg-[#DC143C] text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>
                    {v}
                  </button>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 fade-slide-up" style={{ animationDelay: '360ms' }}>
          ...
          <div className="glass-card rounded-[24px] border border-white/10 overflow-hidden flex flex-col min-h-[360px]">
          ...
          <div className="flex flex-col gap-5">
            {/* Próximas Graduações */}
            <div className="glass-card rounded-[24px] border border-white/10 overflow-hidden flex flex-col flex-1">
              <div className="p-6 border-b border-white/5 bg-[#0a0a0a]/50 flex justify-between items-center">
                <h3 className="text-sm font-bold uppercase tracking-widest text-white flex items-center gap-2">
            ...
            {/* Alertas */}
            <div className="glass-card rounded-[24px] border border-[#DC143C]/20 overflow-hidden bg-gradient-to-b from-[#DC143C]/5 to-transparent">
